    ...
    return (
        <div className="py-xl border-b border-border/50 bg-bg">
            <div className="container-custom">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 lg:grid-cols-4 gap-md md:gap-lg">
                    {items.map((item, idx) => (
                        <div
                            key={idx}
                            className={`
                group relative bg-card border border-card-border p-xl rounded-2xl 
                transition-all duration-300 ease-out hover:-translate-y-2 
                hover:shadow-xl hover:border-primary/30 flex flex-col h-full overflow-hidden
                col-span-1 sm:col-span-2 lg:col-span-1
                ${items.length % 2 !== 0 ? 'sm:last:col-start-2 lg:last:col-start-auto' : ''}
            `}
                        >
                            ...
                            <h3 className="text-xl font-bold mb-md text-foreground group-hover:text-primary transition-colors duration-300">
                                {item.title}
                            </h3>

                            <p className="opacity-70 leading-relaxed text-[0.9375rem] flex-grow">
                                {item.description}
                            </p>
